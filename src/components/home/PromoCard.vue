...
<template>
  <div class="mx-4 rounded-2xl overflow-hidden relative bg-bg-card min-h-[160px]">
    <button
      class="absolute top-1.5 right-1.5 p-1.5 rounded-full bg-bg-surface/50 flex items-center justify-center z-10"
    >
    ...
    <div class="flex items-stretch">
      <div class="flex-1 p-5 flex flex-col justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <component :is="partner.component" class="w-6 h-6 text-text-primary" />
          </div>
        </div>
      </div>

      <div class="w-1/2 relative overflow-hidden">
        <img :src="CashBackImage" alt="Cash Back" class="w-full h-full object-cover object-right" />
      </div>
    </div>
  </div>
</template>
