    ...
    <div class="flex gap-4 overflow-x-auto hide-scrollbar py-2">
      ...
      >
        <div 
          ...
          :class="story.isPinned ? 'ring-2 ring-offset-2 ring-offset-bg-primary ring-text-secondary' : 'ring-2 ring-offset-2 ring-offset-bg-primary ring-border'"
        >
          <div class="w-14 h-14 rounded-full bg-bg-surface flex items-center justify-center overflow-hidden">
            <img 
              v-if="story.image" 
              :src="story.image" 
              :alt="story.label"
          ...
          <div v-if="story.isPinned" class="absolute -top-0.5 -right-0.5 w-5 h-5 bg-bg-card rounded-full flex items-center justify-center shadow-lg z-10">
            <PushPinIcon class="w-3.5 h-3.5 text-text-primary" />
          </div>
        </div>
        <span class="text-xs text-text-secondary text-center leading-tight whitespace-normal">{{ story.label }}</span>
      </button>
    </div>
  </div>
</template>
