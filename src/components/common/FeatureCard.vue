...
<template>
  <div class="relative rounded-3xl p-5 min-h-[220px] overflow-hidden" :class="bgColor">
    <div class="relative z-10">
      <div v-if="logos.length" class="flex items-center gap-2 mb-4">
        <div v-for="(logo, index) in logos" :key="index" class="h-6 flex items-center">
      ...
      <div
        v-if="icon"
        class="w-10 h-10 rounded-xl bg-bg-card/20 flex items-center justify-center mb-3 text-2xl"
      >
        {{ icon }}
      </div>
